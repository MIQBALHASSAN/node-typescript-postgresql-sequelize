...
import { NODE_ENV, HOST, PORT, LOG_FORMAT, ORIGIN, CREDENTIALS } from '@src/config';
...
export class App {
  ...
  public listen() {
    this.app.listen(this.port, () => {
      ...
      logger.info(`🚀 Threshold360 server is started at http://${this.host}:${this.port}`);
      logger.info(`🚀 Threshold360 REST API docs is available at http://${this.host}:${this.port}/api/docs`);
      logger.info(`=================================`);
    });
  }
  ...
  private async connectToDatabase() {
    try {
      ...
      logger.info(`=================================`);
      logger.info(`🚀 Threshold360 Database server is Started Successfully!`);
      logger.info(`=================================`);
    } catch (error) {
    ...
    const options = {
      swaggerDefinition: {
        info: {
          title: 'REST API',
          version: '3.0.0',
          description: 'Threshold360 API docs',
        },
      },
      // apis: ['users.yaml'],
